...
namespace cvsba {
    ...
    double  Sba::run
    (
        std::vector<cv::Point3f>& points, //positions of points in global coordinate system (input and output)
...
#define _MK_QUAT_FRM_VEC(q, v){                                     \
  ...
  (q)[0]=sqrt(1.0 - (q)[1]*(q)[1] - (q)[2]*(q)[2]- (q)[3]*(q)[3]);  \
     ...
     */
    double Sba::run(
        std::vector<cv::Point3d>& points, //positions of points in global coordinate system (input and output)
    ...
    )  throw (cv::Exception)
    {
        ...
        { // in this case, rotation is not estimated, so it is filled with initial values
            for (int i = 0; i < nframes; i++)
            {
                cv::Mat Rnormquat;
